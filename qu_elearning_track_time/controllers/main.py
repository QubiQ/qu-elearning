...
class WebsiteSlidesQu(WebsiteSlides):
    ...
    @http.route(['/slides/update_invested_time'], type='json', auth='public',
                website=True)
    def update_invested_time(self, **post):
        user = request.env['res.users'].browse(request.uid)
        slide_id = int(post.get("slide_id"))
        slide_obj = request.env['slide.slide.partner'].sudo().search(
            [('slide_id', '=', slide_id),
        ...
        if slide_obj:
            ...
            slide_obj.create({'partner_id': user.partner_id.id,
                              'slide_id': slide_id,
                              'invested_time': time})
