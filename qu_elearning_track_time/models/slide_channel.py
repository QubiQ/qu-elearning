...
class SlideChannelPartner(models.Model):
    ...
    time_invested = fields.Float(
        string="Minutes", compute="_compute_time_invested")

    def _compute_time_invested(self):
        for rec in self:
            rec.time_invested = sum(
    ...
    def get_slide_time(self):
        return {
            ...
            "view_id": self.env.ref(
                "qu_elearning_track_time.slide_slide_partner_tree_view"
            )[0].id,
            "domain": [
...
class SlideChannel(models.Model):
    ...
    def compute_total_time(self):
        for rec in self:
            rec.total_time_invested_count = sum(
                self.env["slide.slide.partner"]
